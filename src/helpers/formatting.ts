...
/**
 * Converts a hex color string from RRGGBBAA to XML format #AARRGGBB.
 * Input hex should be 8 characters (RRGGBBAA) without prefix.
 */
export function toXmlColorHex(hex: string): string {
  const clean = hex.replace(/^#/, "")
  if (clean.length < 8) {
    return `#FF${clean.substring(0, 6).toUpperCase()}`
  }
  const red = clean.substring(0, 2)
  const green = clean.substring(2, 4)
  const blue = clean.substring(4, 6)
